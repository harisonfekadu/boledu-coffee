<section class="hero_warp">
    ...
    <?php if (!empty($settings['video_url'])) : ?>
        <div class="video-background">
            <div id="bgndVideo-<?php echo $video_id; ?>" class="player" data-property="{videoURL:'<?php echo $settings['video_url']; ?>',containment:'.hero_warp', showControls:false, autoPlay:true, loop:true, mute:true, startAt:0, opacity:0.2, addRaster:true, quality:'default'}"></div>
        </div>
    <?php endif; ?>
...
<script>
    (function ( $ ) {
        ...
        $(document).ready(function() {
            function bgVideo(){
                jQuery("#bgndVideo-<?php echo $video_id; ?>").YTPlayer();
            }
            bgVideo();
        });
    }( jQuery ));

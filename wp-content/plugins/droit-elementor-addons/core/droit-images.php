...
if (!class_exists('Images')) {
    ...
    /**
     * Print post thumbnail by id in loop.
     * Feature added by : DroitLab Team
     * @since : 1.0.0
     * @param array $args
     */
    ...
    /**
     * Print post thumbnail url by id
     * Feature added by : DroitLab Team
     * @since : 1.0.0
     * @param  array  $args
     * @return string
    ...
    public static function get_elementor_attachment( array $args ) {
        ...
        if ( empty( $settings ) ) {
            _doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), esc_html__( 'Cannot get attachment because missing elementor widget settings.', 'droit-elementor-addons' ), '1.0.0' );

            return false;
        }

        if ( empty( $settings["{$image_key}"] ) ) {
            _doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), sprintf( esc_html__( 'Cannot get attachment because image key: %s is not exits.', 'droit-elementor-addons' ), $image_key ), '1.0.0' );
        ...
        if ( empty( $image['url'] ) && empty( $image['id'] ) ) {
            _doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), sprintf( esc_html__( 'Cannot get attachment because image key: %s has no both id & url.', 'droit-elementor-addons' ), $image_key ), '1.0.0' );
    ...
    public static function image_placeholder( $width, $height ) {
        echo '<img src="' . DROIT_EL_ADDONS_IMAGE . '/placeholder.png" width="'.$width.'" height="'.$width.'" alt="' . esc_attr__( 'Thumbnail', 'droit-elementor-addons' ) . '"/>';
        }
    }
}
